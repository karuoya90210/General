/* table 50100 "Approval Entry Custom"
{
    ...
    fields
    {
        ...
        field(9; Status; Option)
        {
            ...
            trigger OnValidate()
            begin
                if (xRec.Status = Status::Created) and (Status = Status::Open) then "Date-Time Sent for Approval" := CreateDateTime(Today, Time);
            end;
        }
        ...
        {
            CalcFormula = Exist("Approval Comment Line" WHERE("Table ID" = FIELD("Table ID"), "Record ID to Approve" = FIELD("Record ID to Approve"), "Workflow Step Instance ID" = FIELD("Workflow Step Instance ID")));
            Caption = 'Comment';
            Editable = false;
        ...
        {
            CalcFormula = Count("Approval Entry" WHERE("Record ID to Approve" = FIELD("Record ID to Approve"), Status = FILTER(Created | Open), "Workflow Step Instance ID" = FIELD("Workflow Step Instance ID")));
        ...
        {
            CalcFormula = Count("Approval Entry" WHERE("Record ID to Approve" = FIELD("Record ID to Approve"), Status = FILTER(Approved), "Workflow Step Instance ID" = FIELD("Workflow Step Instance ID")));
            Caption = 'Number of Approved Requests';
            FieldClass = FlowField;
        }
        field(27; "Number of Rejected Requests"; Integer)
        {
            CalcFormula = Count("Approval Entry" WHERE("Record ID to Approve" = FIELD("Record ID to Approve"), Status = FILTER(Rejected), "Workflow Step Instance ID" = FIELD("Workflow Step Instance ID")));
        ...
        {
            CalcFormula = Exist("Workflow - Record Change" WHERE("Workflow Step Instance ID" = FIELD("Workflow Step Instance ID"), "Record ID" = FIELD("Record ID to Approve")));
    ...
    var
        RecRef: RecordRef;
    begin
        if not RecRef.Get("Record ID to Approve") then exit;
        RecRef.SetRecFilter;
    ...
    begin
        if not RecRef.Get("Record ID to Approve") then exit(RecNotExistTxt);
        ...
        case RecRef.Number of
            DATABASE::"Sales Header":
                begin
                    ...
                    Details := StrSubstNo('%1 ; %2: %3', SalesHeader."Sell-to Customer Name", SalesHeader.FieldCaption(Amount), SalesHeader.Amount);
                end;
            DATABASE::"Purchase Header":
                begin
                    RecRef.SetTable(PurchHeader);
                    PurchHeader.CalcFields(Amount);
                    Details := StrSubstNo('%1 ; %2: %3', PurchHeader."Buy-from Vendor Name", PurchHeader.FieldCaption(Amount), PurchHeader.Amount);
                end;
            else
    ...
    procedure GetCustVendorDetails(var CustVendorNo: Code[20];
    var CustVendorName: Text[100])
    var
    ...
    begin
        ...
        if WorkflowRecordChange.FindSet then
            repeat
                ...
                ChangeDetails += StrSubstNo(ChangeRecordDetailsTxt, WorkflowRecordChange."Field Caption", OldValue, NewValue);
    ...
    var
        UserSetup: Record "User Setup";
    begin
        if UserSetup.Get(UserId) and UserSetup."Approval Administrator" then exit;
    ...
    local procedure OnAfterGetRecordDetails(RecRef: RecordRef;
    ChangeRecordDetails: Text;
    var Details: Text)
    begin
    end;
